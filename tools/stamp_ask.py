...
(ask/request-box/straight-through/no-guide). The asker is told by
`/attention`, which has watched the `asks` var across a builder's op since
2026-08-23 and picks the channel — the open panel updates in place, a
foreground app pulses the lozenge, a backgrounded one gets a notification
carrying the note. This tool sends nothing itself.
...
def main():
    ...
    ap.add_argument("--status",
                    ...
                    help="the ladder: asked -> proposed -> building -> shipped, "
                         ...
                         "how to do it, and /attention takes it to the asker)")
    ...
    for user in users:
        ...
        stamped += 1
        # nothing rings from here. The op-door write above IS the ring:
        # /attention watches the `asks` var across a builder's op and sends
        # the changed entry's question or note to the owner's subscriptions
        # by its own three-channel rule — panel open, in place; foreground,
        # a pulse; backgrounded, a notification — with /present holding the
        # push back for someone who is looking at the app. Ringing here as
        # well rang twice and ignored that rule (found on the rig,
        # 2026-09-04; see the note in notes.md).
    if not stamped:
