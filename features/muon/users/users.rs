...
impl feature_Users {
    fn auth_dir() -> String {
        format!("{}/.muon-auth", std::env::var("HOME").unwrap_or_default())
    }
    ...
    // the guest list: ~/.muon-auth/users.json, [{ "name": "...", "phone": "+..." }],
    ...
    fn random_bytes(n: usize) -> Vec<u8> {
        ...
        let mut f = std::fs::File::open("/dev/urandom").expect("muon: no /dev/urandom");
        let _ = f.read_exact(&mut buf);
        buf
    }

    // 32-byte signing secret, generated once into ~/.muon-auth/secret
    fn secret() -> Vec<u8> {
