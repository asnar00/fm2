  ...
  async run() {
    try {
      if (window.PublicKeyCredential && !localStorage.muonPasskey) {
        ...
        if (o.ok) {
          ...
          if (rr.ok) { localStorage.muonPasskey = "1"; this.log("passkey enrolled"); }
    ...
    try {
      ...
      if ("PushManager" in window && standalone && !localStorage.muonPush) {
        ...
        if (rr.ok) { localStorage.muonPush = "1"; this.log("push enrolled"); }
