...
The first rung of the graded transcription ladder (`/dictate` #p36–39), built first by choice — it is the offline-true floor. Ticked, it makes `transcribe_local()` reachable: recordings gain a draft transcript computed entirely on the device (whisper-tiny via transformers.js — WebGPU when the device has it, wasm otherwise; the engine and model are served from muon's own origin, never a third-party CDN). The scheduler stamps the result grade 1 — *a draft, honestly labelled* — and when a better rung (server, api) comes into reach later, the same recording is re-derived and upgraded in place. Transcription is compute, not hardware: it still must not run during `/replay`, because re-enactment sends no events.
...
Device choice survived contact with the field (#p63: a real iPhone handed over a webgpu adapter, and ort's webgpu backend then failed). The diagnosis, corrected by #p64: **the phone's GPU is fine** — ftr's haze runs raw WebGPU (Rust/wgpu) nicely on the same iPhone — so the failure is specific to onnxruntime-web's webgpu (jsep) integration in our setup, the same stack that refuses to initialize in a module worker. The fallback is therefore the right *behaviour* but not the end of the story: **making ort's webgpu path actually work on iOS is a named refinement with a real prize (~5-10x faster transcription)**, likely starting from whatever jsep needs that a bare `import` doesn't give it (its worker-spawn from `ort.webgpu.bundle.min.mjs` is the prime suspect).

Mechanically: the probe demands a full adapter **and** device grant before trusting webgpu (the simulator showed Safari exposing `navigator.gpu` with a null adapter — claims without capability exist in both directions), and on any webgpu failure, load-time or run-time, the engine escapes v4's memoized-failure trap by re-importing transformers with a cache-busting query (a fresh module is a fresh memo), rebuilding on wasm, retrying once, and pinning the working device in `localStorage.muonSttDevice` so later sessions skip the broken path. Verified three ways: headless Chrome wasm path (~2.7s, correct text), headless Chrome with webgpu *forced* where none exists (fallback rescues, correct text, device pinned), and iOS Simulator WebKit (~2.5s, correct text). Fallback costs, accepted: the rescue re-fetches the model files that session, and a pinned `wasm` means the phone won't retry webgpu even after the jsep refinement lands — clearing the pin is part of that future node's job.
