...
Device choice survived contact with the field (#p63: a real iPhone handed over a webgpu adapter whose ort backend then failed). The probe now demands a full adapter **and** device grant before trusting webgpu — the simulator showed Safari claiming `navigator.gpu` with a null adapter, its own flavour of the same lie — and on any webgpu failure, load-time or run-time, the engine escapes v4's memoized-failure trap by re-importing transformers with a cache-busting query (a fresh module is a fresh memo), rebuilding on wasm, retrying once, and pinning the working device in `localStorage.muonSttDevice` so later sessions skip the broken path. Verified three ways: headless Chrome wasm path (~2.7s, correct text), headless Chrome with webgpu *forced* where none exists (fallback rescues, correct text, device pinned), and iOS Simulator WebKit (~2.5s, correct text). The fallback's one cost, accepted: the rescue re-fetches the model files that session.
