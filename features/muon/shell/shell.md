...
`shell.rs` defines the `render() -> String` chain base (lines 5-7, returns empty). `assets/index.html` is the loader: fetches `client.wasm`, instantiates it, calls the exported `fm_entry()`, unpacks the returned ptr/len pair from wasm memory, and sets `#app`'s HTML; registers `sw.js`. After paint it checks the deploy stamp (`site/version`, written by deploy.sh from the git hash) against the one it launched from — on change it drops the cache and reloads once, so a deploy is picked up on the next launch. `assets/sw.js`: network-first for everything — the cache is only consulted when the network is unreachable (basic principle: online always means current); every successful fetch refreshes the offline copy; `/auth/*` and `version` bypass the cache entirely. `assets/manifest.json` + icon PNGs make the app installable, standalone, black-themed.
