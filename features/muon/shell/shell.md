...
`shell.rs` defines the `render() -> String` chain base (lines 5-7, returns empty). `assets/index.html` is the loader: fetches `client.wasm`, instantiates it, calls the exported `fm_entry()`, unpacks the returned ptr/len pair from wasm memory, and sets `#app`'s HTML; registers `sw.js`. After paint it checks the deploy stamp (`site/version`, the commit-count build number written by deploy.sh) against the one it launched from — on change it drops the cache and reloads once. A small **logo button** bottom-right (hamburger stand-in; safe-area-inset positioned) is the **system panel**'s handle: tapping it shows who's logged in, the running build, recent changes (`changes.json`, generated at deploy from commit subjects), and enrolment / log out / update buttons. Build numbers live in the panel only — the button shows no number, and pulses gently (accent colour) when a newer build is waiting. Returning to foreground, regaining connectivity, and a 60s visible-poll all re-check. `assets/sw.js`: network-first with a freshness deadline (1.2s) — a fresh copy that arrives in time always wins; if a cached copy exists and the network is slower than the deadline, the cache serves and the fetch completes in the background, refreshing the offline copy; offline is the deadline missed instantly; nothing cached means waiting on the network however long. `/auth/*` and `version` bypass the cache entirely. `assets/manifest.json` + icon PNGs make the app installable, standalone, black-themed.
