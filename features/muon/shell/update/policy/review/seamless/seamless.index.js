const feature_Seamless = {
  ...
  rehydrate() {
    let stash = null;
    try { stash = JSON.parse(localStorage.muonStash || 'null'); } catch (e) {}
    delete localStorage.muonStash; // consumed once, matching or not
    if (!stash || String(stash.v) !== String(localStorage.muonVersion)) return;
    let fresh = {}, old = {};
...
};
{
  if (typeof feature_Review !== 'undefined') {
    ...
    feature_Review.apply = async function (build) {
      ...
      try {
        localStorage.muonStash = JSON.stringify(
