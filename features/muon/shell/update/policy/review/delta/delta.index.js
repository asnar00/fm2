const feature_Delta = {
  ...
  stored() {
    try { return JSON.parse(localStorage.muonHashes || 'null'); } catch (e) { return null; }
  },
  store(m) {
    try { localStorage.muonHashes = JSON.stringify(m); } catch (e) {}
  },
  fetchLive() {
  ...
  async evict() {
    ...
    try {
      const cache = await caches.open('muon');
