...
The panel gains a `view source` button that opens the live feature tree at `/features/` — muon's own self-description, served by muon. It opens outside the PWA window (a new tab / in-app browser) so the app itself stays put. A feature-modular app's "source" is its feature tree, not a file listing: the button lands on the same orientation page any agent or human uses.
