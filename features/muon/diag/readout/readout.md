...
`readout.rs`: a `route` /extension/ — `POST diag/readout` stores the latest snapshot (size-capped, overwriting `/tmp/muon-readout.json`), `GET` returns it; both refuse tunnel traffic without a valid cookie, while localhost stays open for tooling.
