...
Once per device: tap the build number → "enable notifications" → allow. From then on, deploys announce themselves as notifications — app closed or not. Tapping the notification opens miso.
...
- **VAPID**: proof to the push relay that the sender is miso's server, via a signed token.
