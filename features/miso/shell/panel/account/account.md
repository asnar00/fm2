...
`account.js` is the page half. It wraps `feature_Loop.apply` and watches the `open_tool` local var: on transition into `account` it calls `openTool()`, on transition out, `closeTool()` — this node's own open seam, whose defaults are the panel sheet opening and closing (both typeof-guarded) exactly as they always did. A feature that gives 👤 a surface of its own replaces the pair rather than editing this file; `/me` is the first to do so. It wraps `feature_Panel.close` to call `dismissed()`, this node's dismissal seam: by default a shade-tap, when the account tool is still open, also sends `tools_home` — it only fires while `open_tool` is `account`, so the close that follows the resulting state change cannot loop. A feature that gives 👤 its own surface replaces `dismissed` (`/stay`, 2026-08-25). It takes the corner button's seam (`feature_Panel.buttonTap = () => {}`): the tap is parked for the future agent interface.
