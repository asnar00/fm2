...
`shell.rs` defines the `render(state) -> String` chain base (returns empty HTML, drawn from the `/loop` loop's state); subfeatures extend it with content.

`assets/index.html` is the loader skeleton: it registers the service worker, walks the launch flow (install redirect, whoami, then hands the app to `feature_Loop.boot()`), with every feature reference typeof-guarded.
