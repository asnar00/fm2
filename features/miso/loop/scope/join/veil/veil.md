...
- **veil**: the cover between boot and first reveal, lifted by the join
  snapshot or the timeout.

## code description

`veil.rs` extends `update`: when the `VarJoin` snapshot has been applied by
...
`veil.index.js` owns the veil: it covers the page at load, wraps
...
`veil.index.css` hides `#app` until the body carries `fm-joined`, and styles
